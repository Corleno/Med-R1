...
# Example: bash scripts/evaluation/train_MRI/qwen/think_ablation/eval_qwen3vl_235b_a22b_kdpo_think_tau_0_ablation_mri_600_all_modalities_parallel.sh

MODEL_PATH="/mnt/task_runtime/output/MM-GOLD/Qwen3-VL-2B-KDPO-MRI-600-think-tau-0-235b-a22b"
BATCH_SIZE=16
MODE="think"
...
for CHKPT_PATH in "${CHECKPOINTS[@]}"; do
    ...
    if [[ "${CHKPT_BASENAME}" == checkpoint-* ]]; then
        MODEL_NAME="Qwen3-VL-2B-KDPO-MRI-600-think-tau-0-235b-a22b_${CHKPT_BASENAME}"
    else
        MODEL_NAME="Qwen3-VL-2B-KDPO-MRI-600-think-tau-0-235b-a22b"
    fi
